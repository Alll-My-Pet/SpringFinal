...
$(document).ready(function () {
    ...
    // menuToggle.addEventListener('click', function () {
    //     // overallDropdown.style.display = overallDropdown.style.display === 'block' ? 'none' : 'block';
    //     if (overallDropdown.style.display === 'flex') {
    //         overallDropdown.style.display = 'none'; // 드롭다운 숨김
    //     } else {
    //         overallDropdown.style.display = 'flex'; // 드롭다운 표시
    //     }
    // });

    // // 전체 드롭다운 외부 클릭 시 닫기 기능
    // document.addEventListener('click', function (event) {
    //     if (!menuToggle.contains(event.target) && !overallDropdown.contains(event.target)) {
    //         overallDropdown.style.display = 'none';
    //     }
    // });
    // 드롭다운 내부에 마우스가 들어오면 드롭다운 숨기지 않도록 처리
