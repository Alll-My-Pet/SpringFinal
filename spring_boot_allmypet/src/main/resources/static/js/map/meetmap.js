...
$(document).ready(function() {
    ...
    // 필터링 함수
    window.filterMeets = function() { // Make it globally accessible
        const addressRegion = document.getElementById('addressRegion').value;
        const addressDo = document.getElementById('addressDo1').value;
        const addressSiGunGu = document.getElementById('addressSiGunGu1').value;
        const petCtgNo = document.getElementById('petCtgNo').value;

        const meetTypes = Array.from(document.querySelectorAll('input[name="meetType"]:checked')).map(checkbox => checkbox.value);

        const url = `/meet?addressRegion=${addressRegion}&addressDo=${addressDo}&addressSiGunGu=${addressSiGunGu}&petCtgNo=${petCtgNo}&meetType=${meetTypes.join(',')}&searchKeyword=${document.getElementById('searchKeyword').value}`;

        window.location.href = url;
    };

    // 모든 모임 표시 함수
    function showAllMeets() {
        let url = '/meet';
        window.location.href = url; 
    }
    ...
    document.getElementById('filterButton').addEventListener('click', filterMeets);
	document.getElementById('showAllButton').addEventListener('click', showAllMeets);
});